...
def capt_process(captcha_path, train_path):
    files = os.listdir(captcha_path)
    for i, file in enumerate(files, 1):
            img = Image.open(captcha_path + file)
            width, height = img.size
            img = img.convert('L')
            img = binarizing(img)
            cuts = vertical(img)
            start_x = get_start_x(cuts)
            # 4 开始滴水算法
            start_route = []
            ...
            img1.save(train_path + str(2 * i - 1) + '.png')       
            ...
            img2.save(train_path + str(2 * i) + '.png')
